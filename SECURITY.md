...
The right-click diagnostic center is intentionally redacted: it reports only fixed checks, connection mode and safe status metadata, never tokens, account IDs, proxy addresses or full API responses. Startup is stored as a per-user Windows Run entry and does not require administrator access.
