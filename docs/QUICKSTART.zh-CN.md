...
> 面向日常使用的完整操作请先看 [中文用户操作手册](USER-GUIDE.zh-CN.md)；本文保留安装、源码构建和回归测试细节。
...
状态栏窗口不会出现在 `Alt+Tab` 切换列表中。状态栏会以低频状态呼吸和固定高光保持可见反馈；点击空白区域或展开按钮进入 Usage Hub 后，可以看到环形进度、趋势线、固定位置的律动脉冲和刷新旋转。开启“忽略鼠标操作（点击穿透）”后，状态栏只展示不抢鼠标，点击会直接交给背后窗口；从通知区域托盘菜单即可取消。点击右上角 `×` 会隐藏到 Windows 通知区域，双击托盘图标可以恢复；只有在托盘或右键菜单选择“退出”才会结束程序。

## 开机自启与诊断

第一次启动会默认开启当前 Windows 用户的开机自启，不需要管理员权限。状态栏右键菜单中的“开机自启”可以随时关闭或重新开启；程序会记住你的选择，不会在下次启动强行改回。

右键选择“诊断中心”会先重新查询一次，再显示 OAuth、网络代理、额度查询、窗口数量、本地缓存、历史、趋势预测、全局快捷键和启动项等固定检查项，并给出下一步建议。窗口内可以重新检查或复制安全摘要，其中不包含 Token、账户 ID、代理地址或完整响应。

右键选择“设置”可以调整自动刷新周期（1/5/10/15/30/60 分钟）、历史保留周期（7/30/90 天）、跟随系统/深色/浅色/石墨主题、背景实色或三档透明度、是否开启“忽略鼠标操作（点击穿透）”展示模式、开机后的首次查询延迟、是否在启动时检查更新（仅提示，不自动安装）、是否记住窗口位置、是否开启平滑动效、全局快捷键，以及是否在额度达到阈值、周期重置或预计两小时内耗尽时弹出通知。平滑动效默认开启，会让进度条和刷新图标自然过渡；关闭后会立即回到静态绘制。
...
| 全局快捷键 | `Ctrl+Alt+U` 唤起或聚焦 Usage Hub |
| 复制摘要 | Usage Hub 内按 `Ctrl+C`，或点击“复制摘要” |
| 导出趋势 | Usage Hub 内按 `Ctrl+E`，或右键选择“导出本地趋势” |
| 仅展示模式 | 在设置或右键菜单开启“忽略鼠标操作（点击穿透）”；从托盘菜单取消 |
| 设置、选项与诊断 | 右键状态栏或托盘图标 |

程序默认每 5 分钟自动刷新一次，也可以在设置中选择其他周期。开机自启时可设置 0/5/15/30 秒延迟，让代理和 Codex CLI 有时间完成初始化；开启“启动时检查更新”后只会在首次查询完成后提示公开 Release，不会自动下载或替换文件。拖动过窗口后，自动定位不会再抢回你选择的位置；开启“记住上次位置”后，下次启动会恢复该位置。右键“检查更新”只读取公开 Release 元数据，确认后打开 GitHub 页面，不会后台覆盖运行中的文件。导出的 CSV 位于应用数据目录的 `exports` 子目录，只包含窗口秒数、百分比和时间；右键“打开数据目录”可以直接查看。清除趋势历史会连同导出副本一起删除，清除缓存不会影响趋势文件。
...
- 本地缓存和按设置保留 7/30/90 天的历史只保存 Provider、窗口秒数、百分比和时间；导出的 CSV 会去掉 Provider，只保留窗口秒数、百分比和时间。右键菜单可单独删除趋势历史与导出，或单独删除最近成功缓存。
