...
状态栏窗口不会出现在 `Alt+Tab` 切换列表中。状态栏会以低频状态呼吸和进度高光保持可见反馈；点击空白区域或展开按钮进入 Usage Hub 后，可以看到环形进度、趋势线、扫描线和刷新旋转。开启“忽略鼠标操作（点击穿透）”后，状态栏只展示不抢鼠标，点击会直接交给背后窗口；从通知区域托盘菜单即可取消。点击右上角 `×` 会隐藏到 Windows 通知区域，双击托盘图标可以恢复；只有在托盘或右键菜单选择“退出”才会结束程序。
