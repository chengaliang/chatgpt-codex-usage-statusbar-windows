...
右键选择“设置”可以调整自动刷新周期（1/5/10/15/30/60 分钟）、历史保留周期（7/30/90 天）、跟随系统/深色/浅色/石墨主题、35%–100% 背景不透明度滑块、是否开启“忽略鼠标操作（点击穿透）”展示模式、开机后的首次查询延迟、是否在启动时检查更新（仅提示，不自动安装）、是否记住窗口位置、是否开启平滑动效、全局快捷键，以及是否在额度达到阈值、周期重置或预计两小时内耗尽时弹出通知。拖动滑块即可实时预览状态栏，保存后保留新值，取消则恢复原值。平滑动效默认开启，会让进度条和刷新图标自然过渡；关闭后会立即回到静态绘制。
